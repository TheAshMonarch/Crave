...
def save_favorite(recipe_id):
    from database import add_favorite, get_recipe_by_id
    if 'user_id' not in session:
    ...
    try:
        ...
        app.logger.info(f"Favorite added for recipe_id: {recipe_id}")
        return jsonify({'status': 'success', 'action': 'added'})
    except sqlite3.IntegrityError:
        app.logger.error(f"Favorite already exists for recipe_id: {recipe_id}")
        return jsonify({'status': 'error', 'message': 'Favorite already exists'}), 400
    except Exception as e:
        app.logger.error(f"Save favorite error: {str(e)}")
